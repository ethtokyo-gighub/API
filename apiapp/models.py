...
class User(AbstractUser):
    ...
    next_paycheck_amount = models.FloatField(default=5000, blank=True)
...
class Loan(models.Model):
    ...
    borrowing_amount = models.FloatField(default=100, blank=True)
    ...
    def __str__(self):
        return "user: " + self.user.address[:5] + " / org: " + self.organization.title + " / amount: " + str(self.borrowing_amount)
        
    def get_max_borrowing_amount(self):
        result = self.LTV*self.user.next_paycheck_amount*0.01
        return result

    def save(self, *args, **kwargs):
        self.max_borrowing_amount = self.get_max_borrowing_amount()
